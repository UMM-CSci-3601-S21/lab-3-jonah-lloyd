  ...
  describe('getTodos()', () => {
    ...
    describe('Calling getTodos() with parameters correctly forms the HTTP request', () => {
      ...
      it('correctly calls api/todos with multiple filter parameters', () => {
        ...
        todoService.getTodos({ status: false, category: 'software design', owner: 'Blanche' }).subscribe(
          todos => expect(todos).toBe(testTodos)
        );
        ...
        const req = httpTestingController.expectOne(
          (request) => request.url.startsWith(todoService.todoUrl)
            && request.params.has('status') && request.params.has('category') && request.params.has('owner')
        );
